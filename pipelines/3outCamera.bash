...
queue ! nvtracker tracker-width=640 tracker-height=368 \
  ll-lib-file=/opt/nvidia/deepstream/deepstream-6.0/\
lib/libnvds_nvmultiobjecttracker.so \
  ll-config-file=/opt/nvidia/deepstream/deepstream-6.0/\
samples/configs/deepstream-app/config_tracker_IOU.yml \
  enable-batch-process=1 ! \
queue ! nvinfer name=secondary-infer unique-id=2 \
  process-mode=2 infer-on-gie-id=1 infer-on-class-ids="0:" \
  batch-size=16 \
  config-file-path=/opt/nvidia/deepstream/deepstream-6.0/\
